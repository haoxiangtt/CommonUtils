...
public class 	NetworkState {
	...
	public static final int NETWORK_TYPE_CMNET = 0;// 移动网络数据流量
	public static final int NETWORK_TYPE_CMWAP = 1;// 移动2G网络
	public static final int NETWORK_TYPE_WIFI = 2;//wifi网络
	public static final int NETWORK_TYPE_MOBILE = 3;//数据流量
	public static final int NETWORK_TYPE_MOBILE_WIFI = 4;//wifi mobile同开
	...
	private int initNetWorkStatus(Context context) {
		if (context != null) {
			...
			if (info != null && info.getType() == ConnectivityManager.TYPE_WIFI) {
				...
				if (wifiState != null && State.CONNECTED == wifiState) {
					// 无线网络连接成功
					this.connected = true;
					if (isMobileEnabled(cm)) {
						return NETWORK_TYPE_MOBILE_WIFI;
					} else {
		...
		LogUtil.d(TAG, "=====初始网络状态==>>"+connected);
		return NETWORK_TYPE_NONE;

	}

	private boolean isMobileEnabled(ConnectivityManager connectivityManager) {
		try {
			Method getMobileDataEnabledMethod = ConnectivityManager.class.getDeclaredMethod("getMobileDataEnabled");
			getMobileDataEnabledMethod.setAccessible(true);
			boolean isDataOn = (Boolean) getMobileDataEnabledMethod.invoke(connectivityManager);
			LogUtil.d(TAG, "data is on ---------"+isDataOn);
			return isDataOn;

		} catch (Exception e) {
			LogUtil.e(TAG, "data is on ----反射出错-----");
		}
		// 反射失败，默认关闭
		return false;
	}
	...
	public boolean ping(){
		...
		try {
	        ...
	        while((result = read.readLine()) != null){
	          if(DEBUG){ LogUtil.e(TAG, ">>>>>>>"+result); }
	        }
	        int status = p.waitFor();
	        if(DEBUG){ LogUtil.e(TAG,">>>>>>status="+status); }
	        if(status == 0){
	        	return true;
	        }else{
	        	return false;
