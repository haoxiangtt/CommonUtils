...
//    private TelephonyManager mySystemAPITM;
    ...
    public String getImei(int simID) {
        if (currentapiVersion >= 21) {
            ...
            if (myQualcommTMInstance != null) {
                try {
                    return (String) eval(myQualcommTMInstance, "getDeviceId", new Object[]{simID}, new Class[]{int.class});
    ...
    public String getImsi(int simID) {
        try {
            ...
            if (currentapiVersion >= 21) {
                ...
                if (myQualcommTMInstance != null) {
                    return (String) myQualcommTMInstance.getClass().getMethod("getSubscriberId", int.class)
    ...
    public String getOperator(int simID) {
        if (currentapiVersion >= 21) {
            ...
            try {
                return (String)eval(myQualcommTMInstance, "getSimOperator", new Object[]{simID}, new Class[]{int.class});
            } catch (DualSimMatchException e) {}
        }
        return "";
    }
    ...
    private boolean isCMDualSimQualcommSystem(Context myContext) {
        ...
        if (currentapiVersion >= 21) {
            try {
                boolean reflectionIsMultiSimEnable = (Boolean) mTelephonyManager.getClass().getDeclaredMethod("isMultiSimEnabled").invoke(mTelephonyManager);
     ...
     * @return
     */
    private static boolean isMiDualSimQualcommSystem() {
    ...
    private boolean isHuaWeiDualSimQualcommSystem(Context myContext) {
        ...
        if (currentapiVersion >= 21) {
            try {
                boolean reflectionIsMultiSimEnable = (Boolean) mTelephonyManager.getClass().getDeclaredMethod("isMultiSimEnabled").invoke(myContext);
                return reflectionIsMultiSimEnable;
            } catch (Exception e) {
                if (BuildConfig.DEBUG) { e.printStackTrace(); }
            }
    ...
    private static boolean isVivoQualcommSystem() {
        if (!"BBK".equals(Build.MANUFACTURER))
            return false;

        try {
            String execResult = SystemProperties.getProperty(QUALCOMM_VIVO_PLATFORM);
            if (!TextUtils.isEmpty(execResult)) {
    ...
    private static boolean isVivoX5DualSimQualcommSystem() {
        ...
        try {
            String execResult = SystemProperties.getProperty(QUALCOMM_VIVO_X5_PLATFORM);
            if (!TextUtils.isEmpty(execResult)) {
