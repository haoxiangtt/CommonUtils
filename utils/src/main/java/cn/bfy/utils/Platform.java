...
//                LogUtil.e(TAG,"ExecutorService class true,add a new task");
        ...
        public boolean cancel(String sessionId) {
            if (DEBUG) {
                LogUtil.e(TAG,"Begin cancel the thread pool by sessionId : " + sessionId);
            }
            if (TextUtils.isEmpty(sessionId)) {
                if (DEBUG) {
                    LogUtil.e(TAG,"SessionId is empty!");
                }
                return false;
            }
            List<WeakReference<Future<?>>> list = mThreadMap.get(sessionId);
            ...
            if (list != null) {
                if (DEBUG) {
                    LogUtil.e(TAG,"Find " + list.size() + " threads in the list.");
                }
                int i = 0;
                for (WeakReference<Future<?>> ref : list) {
                    if (ref.get() != null) {
                        ...
                        if (!task.isDone()) {
                            if (task.cancel(true)) {
                                if (DEBUG) {
                                    LogUtil.e(TAG, "Cancel a thread successfully!index = " + i);
                                }
                            } else {
                                flag++;
                                if (DEBUG) {
                                    LogUtil.e(TAG, "Cancel a thread failure!index = " + i);
                                }
                            }
                        }
                    }
            ...
            try {
                ...
                if (DEBUG) {
                    LogUtil.e(TAG,"End cancel thread pool!!!!");
                }
            }
